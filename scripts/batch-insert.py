#!/usr/bin/env python3
"""Batch insert 116 Notion projects into Supabase catalog_projects.
Usage: python3 scripts/batch-insert.py
Requires: /tmp/devs.json (output of developer query), ACCESS_TOKEN from mcp.json
"""
...
with open(os.path.expanduser('~/.claude/mcp.json')) as f:
    mcp = json.load(f)
args = mcp['mcpServers']['supabase']['args']
TOKEN = args[args.index('--access-token') + 1]
...
# Load developers
devs_data, err = run_sql("SELECT id, name FROM developers")
if err:
    print(f"Failed to load devs: {err}")
    sys.exit(1)
dev_map = {d['name']: d['id'] for d in devs_data}
print(f"Loaded {len(dev_map)} developers")

# Load existing projects
existing_data, err = run_sql("SELECT lower(trim(name)) as n FROM catalog_projects")
if err:
    print(f"Failed to load existing: {err}")
    sys.exit(1)
existing = set(r['n'] for r in existing_data)
print(f"Existing projects: {len(existing)}")

MAP = {
    'Sunny': 'Sunny Development Group', 'Taryan': 'Taryan Group', 'Cube Group': 'CUBE Development',
    'Baza': 'BAZA Development', 'Tamora': 'Tamora Group', 'Stetsyuk': 'Stetsyuk Development',
    'HQC': 'High Quality Construction', 'NOVO Development': 'Novo Development',
    '618 Developments': '618 Development', 'Mirah': 'Mirah Investment & Development',
    'Mirah I&D': 'Mirah Investment & Development', 'Bali Invesments': 'Bali Investments',
    'Lyvin': 'Lyvin Properties', 'Avalon': 'Avalon Bali Group', 'Predmet': 'Predmet Construction',
    'Coco': 'Coco Development Group', 'Nuanu': 'Nuanu Creative City', 'Unit': 'Unit Space',
    'Oxo': 'OXO Living', 'Azimut': 'Azimut Development', 'Yolla Development': 'Yolla Group',
    'Yolla': 'Yolla Group', 'Bali Baza': 'BAZA Development', 'Anta': 'Anta Group',
    'LOYO': 'Loyo & Bondar Group', 'Archestet': 'ARCHESTET', 'Oceaniq': 'OceaniQ Villas',
    'Pertama': 'Pertama Property', 'Ecolit': 'Ecolit Group / Sansara Development',
    'Vibe': 'Vibe Development', 'Arya': 'Arya Properties', 'Iji': 'IJI Group',
    'Feels': 'Feeels Development', 'PCE': 'Premier Global Development', 'Almal': 'Almal',
}
...
def resolve(n):
    name = MAP.get(n, n)
    return name, dev_map.get(name)

# All 116 projects: (name, notion_dev, files_url, chess_url)
ALL = [
    ...
    ("Sunny Nine","Sunny","https://drive.google.com/drive/folders/1ZsCXaFnvPf3zwSSdw-MqjGamURJB0eYP","https://docs.google.com/spreadsheets/d/12BLaCyooz__PqXLXL8oj07WS4r3RtKnj1-Kkz8EbfTg/edit"),
    ...
    ("Cube Apartment","Cube Group","https://drive.google.com/file/d/1OYsCyg78xmKgPUTsVfSB7JsRkw8aEBCl/view",None),
    ("Aura","Aura Investment","https://drive.google.com/drive/folders/1bs75aWOiuc4ImyBSLQs2QnyFlAgDPczY","https://docs.google.com/spreadsheets/d/1LzfO-EVrHeQ6YLlR9oOROvq4-4DWWMwA/edit"),
    ("Baza Kedungu","Baza","https://drive.google.com/drive/folders/1iTcwBnRFL6WgNoanjZUtAscShj2Od8Dx","https://docs.google.com/spreadsheets/d/1TWbIAHCWka3ChmSxnl_n1brWpvHWygSgQygDn7SoKMI/edit"),
    ...
    ("Dolphin Hill","Dolphin Hill","https://drive.google.com/drive/folders/1Fhbv0KACMueAbntBJ2c1v62Sw5AafZn8","https://docs.google.com/spreadsheets/d/1b3ddis9CEclT5f6N53Jmnfc3JMOmo30UdqxbUzxatEg/edit"),
    ...
    ("Five Oceans","Stetsyuk","https://docs.google.com/spreadsheets/d/1i6VH_AnxsvrjYKVQMW9qRoyCqmhEoe9V/edit","https://docs.google.com/spreadsheets/d/14pVs1Lhp-mtTdr_tdOECTwpbI8ERrHI6oMJGOAApwFY/edit"),
    ...
    ("Scorpia","Elysium Group Bali","https://docs.google.com/spreadsheets/d/1d7a3ujevSyvjS_Sr9D2sBINUZffkH_ogqrqOS1biOKs/edit",None),
    ...
    ("AZORIA","Coco","https://imminent-saltopus-650.notion.site/Eng-COCO-Development-Group","https://docs.google.com/spreadsheets/d/1knQUEpDQQUfZxT5Ds-XFC8ZT41LBYUaNpTWDJEtBx48/edit"),
    ...
    ("Habbi","Balimass","https://www.notion.so/balimassinvest/Habbi-Melasti-Residence","https://docs.google.com/spreadsheets/d/1Zhbghv-PMR63EzkZO5I7s8dL7lpAqg_Xxc-7hNIxSa8/edit"),
    ...
    ("Zamaya","Elysium Group Bali","https://docs.google.com/spreadsheets/d/1d7a3ujevSyvjS_Sr9D2sBINUZffkH_ogqrqOS1biOKs/edit",None),
    ("ARYA Sumba","Arya",None,None),
    ("Village ADVA","ADVA","https://docs.google.com/spreadsheets/d/1mhln25Ukh-6Dw7JGXbtGwRmka5bdjOfQMW5mnXpahA8/edit","https://docs.google.com/spreadsheets/d/1PdxdukH8gMvP5zQo8RE4RrjkcSXAKA6rPEevHfRdlfs/edit"),
    ("Ubud City Renaissance Residence","Bali Capital Group","https://balicapitalbook.notion.site/Bali-Capital-Group-Partners","https://docs.google.com/spreadsheets/d/1m7vUR8SWUMpYSxR4TXMELmuDWvw3eSkKPo20J6NJeX4/edit"),
    ("The Heights","Theia","https://accurate-suede-3dd.notion.site/The-Heights","https://residence.theheights.io/board/112881"),
    ...
    ("The Residences OXO","Oxo","https://drive.google.com/drive/folders/1K-0HwW5VcUGSgnv28ETwad796i3AJ7B7","https://oxoliving.notion.site/AGENTS-2262fa4a3dd98042a3a1ce48af5755bc"),
    ("Nagaya Apartments","Bali Capital Group","https://balicapitalbook.notion.site/Bali-Capital-Group-Partners","https://docs.google.com/spreadsheets/d/1YSWjc1IY9VMkbnshHgh3gx4OKpKMDQFtuwmzApAMsOg/edit"),
    ...
    ("Ubud City BCG","Bali Capital Group","https://balicapitalbook.notion.site/Bali-Capital-Group-Partners",None),
    ...
    ("Ubud Central Oasis","Bali Capital Group","https://balicapitalbook.notion.site/Bali-Capital-Group-Partners","https://docs.google.com/spreadsheets/d/1YSWjc1IY9VMkbnshHgh3gx4OKpKMDQFtuwmzApAMsOg/edit"),
    ("The Pavilions OXO","Oxo","https://drive.google.com/drive/folders/15Rkq_b_aUVvl3gSQk3DysEYXFauguIUZ","https://oxoliving.notion.site/AGENTS-2262fa4a3dd98042a3a1ce48af5755bc"),
    ...
    ("Dreamland Feels","Feels","https://drive.google.com/drive/u/0/folders/1YMc_iQY7ebt38VRz-5P_LPN9ozb9oxmA","https://docs.google.com/spreadsheets/d/1-Vsjey0CPjrZt_wUZSHipy-4yiNrgzUfmWWICT6euB4/edit"),
    ("Pandawa Dream","LOYO","https://loyobondar.notion.site/Pandawa-Dream","https://docs.google.com/spreadsheets/d/1qVnrTG-3_UHIexFcZ8sYRDYtFin_He7tDu5g9NE9pwg/preview"),
    ...
    ("Yolla Nunggalan Residence","Yolla Development","https://drive.google.com/drive/folders/1i-X1aqmGQ4NvEYmXyhDysx5_WZkL2HuU","https://drive.google.com/file/d/1Hj5Od4p_sF_8Q31zYKgsZFkd6Z8zrGpq/view"),
    ...
    ("The Bank OXO","Oxo","https://drive.google.com/drive/folders/1og3tKylfSNcgl0WHaftJi7P85HcbdRoa","https://oxoliving.notion.site/AGENTS-2262fa4a3dd98042a3a1ce48af5755bc"),
    ...
    ("Solace Residence","Solace","https://solacedg.com/solace_residence/",None),
    ...
    ("Maya Solace","Solace","https://solacedg.com/maya_residence/",None),
    ("Azura Vibe","Vibe",None,None),
    ...
    ("Axis One","PCE","https://pcepartners.notion.site/Axis-One-RU","https://docs.google.com/spreadsheets/d/1LGZYDe1xaDEkng3zbWnuIUs4mfi5onGu/edit"),
...
# Filter duplicates
to_insert = [(n, d, f, c) for n, d, f, c in ALL if n.strip().lower() not in existing]
print(f"Projects to insert: {len(to_insert)} (skipping {len(ALL) - len(to_insert)} duplicates)")
...
for bi, batch in enumerate(batches):
    vals = []
    for name, dev_notion, files_url, chess_url in batch:
        sname, did = resolve(dev_notion)
        did_sql = f"'{did}'" if did else "NULL"
        vals.append(f"(gen_random_uuid(), {esc(name)}, {esc(sname)}, {did_sql}, 'pending', {esc(files_url)}, {esc(chess_url)}, now(), now())")

    sql = "INSERT INTO catalog_projects (id, name, developer_name, developer_id, status, drive_folder_url, sheets_url, created_at, updated_at) VALUES\n" + ",\n".join(vals)

    result, err = run_sql(sql)
    if err:
        print(f"Batch {bi+1}/{len(batches)} ({len(batch)} rows): ERROR - {err}")
    else:
        print(f"Batch {bi+1}/{len(batches)} ({len(batch)} rows): OK")
...
# Verify
time.sleep(2)
final, err = run_sql("SELECT count(*) as total, count(developer_id) as with_dev FROM catalog_projects")
if final:
    print(f"\nFinal count: {final[0]['total']} projects, {final[0]['with_dev']} with developer_id")
