  ...
  },
  {
    ...
    examples: [
      ...
      },
      {
        ...
        detail: "$10B оценка, $150M+ ARR. Ex-CEO Salesforce. Заменяет BPO-колл-центры, 40% Fortune 50",
        caseStudy: {
          whatTheyDo: "Sierra — AI-платформа для customer experience. AI-агенты полностью заменяют call-center: обрабатывают возвраты, меняют подписки, решают billing, управляют доставкой. Работает через чат, SMS, WhatsApp, email, голос и даже ChatGPT.\n\nAgent OS — операционная система для агентов с SDK и no-code Agent Studio. Multi-model architecture: «constellation of models» — разные LLM для разных задач (fast для tool-calling, precise для классификации, long-context для reasoning).\n\nК сентябрю 2025 голосовые агенты обогнали текстовые по объёму — сотни миллионов AI-звонков.",
          originStory: "Основатели: Bret Taylor (co-creator Google Maps, CTO Facebook, создатель кнопки Like, CEO Salesforce, chairman OpenAI board) и Clay Bavor (ex-VP Google, руководил Google Labs, AR/VR, Project Starline).\n\nТайминг идеальный: Taylor ушёл из Salesforce в ноябре 2022, основал Sierra в феврале 2023, запустил в феврале 2024 — прямо в бум GenAI.\n\nПочему customer service: Taylor увидел что BPO — $500B+ рынок где LLM-агенты могут полностью заменить людей. Incumbent-ы (Salesforce, Zendesk) заперты в seat-based модели и не могут каннибализировать себя.",
          financials: "• Seed (фев 2024): $110M (Sequoia + Benchmark)\n• Series A (окт 2024): $175M при $4.5B (Greenoaks)\n• Series B (сент 2025): $350M при $10B (Greenoaks)\n• Итого: $635M за 3 раунда\n• ARR: ~$26M (конец 2024) → $100M (ноябрь 2025, за 21 месяц) → $150M+ (январь 2026)\n• $100M ARR за 21 месяц — один из самых быстрых SaaS в истории\n• Клиенты: 40% Fortune 50, 1/4 клиентов = revenue >$10B, 50% = revenue >$1B\n• Агенты касаются >95% покупателей в США",
          howTheyUseIt: "Sierra = Vertical AI Agent для customer service:\n\n1. Outcome-based pricing: клиент платит ТОЛЬКО за решённую проблему. Нет результата — бесплатно. Конфликт интересов Zendesk (seat-based: чем лучше AI, тем меньше seats) отсутствует\n2. BPO replacement: конкурирует с Teleperformance/Concentrix ($200B+), не с Zendesk. Ramp автоматизирует 90% обращений через Sierra\n3. Constellation of Models: не один LLM, а набор — low-latency для простого, high-precision для классификации, long-context для reasoning. Supervision layers проверяют guardrails\n4. Voice-first: 80% customer service = телефон. Sierra нативно обрабатывает голос, конкуренты — нет\n5. Brand-specific: каждый agent обучен на tone, policies, products бренда",
          keyClients: "• WeightWatchers — 70% обращений AI, CSAT 4.6/5\n• Ramp — 90% кейсов автоматизировано\n• SoFi — NPS вырос на 33 пункта\n• Sonos — метрика «time-to-music» для CX\n• Nordstrom, ADT, SiriusXM, Cigna, Chime, Wayfair, Brex, AG1\n\n40% Fortune 50. Profile: enterprise B2C бренды с high volume support.",
          lessonsForUnitbox: "1. Outcome-based pricing снимает барьер: «нет продаж — нет затрат». Unitbox: per qualified lead или per sale\n2. $100M ARR за 21 месяц при узком фокусе (только customer service, только enterprise). Unitbox: не размывать — AI каталог + AI sales agent для Bali\n3. Voice-first: 80% RE коммуникации = WhatsApp/телефон. AI voice agent для Bali buyers — на русском, английском, китайском\n4. Метрика Sonos «time-to-music» → Unitbox: «time-to-viewing» (от контакта до просмотра) или «time-to-contract»\n5. Incumbent lock-in: Zendesk не может каннибализировать seats. Традиционные каталоги не могут автоматизировать — они seat-based. Unitbox disrupts",
        },
      },
      {
